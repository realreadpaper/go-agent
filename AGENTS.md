...
- CLI 示例命令需要默认打印必要调试日志，至少能看到 provider/model、当前轮次、stop_reason、tool_use、tool_result 和规范化后的响应内容摘要；禁止打印完整 API key。
- 原始 provider API JSON 只能通过显式开关启用，例如 `AGENT_TRACE_RAW_API=1`。原始日志必须脱敏并截断，不能默认开启。
